...
# picking a random number  from the dictionary of the list 'data' that represent a 'instagram account' and making a
    ...
    a = list(random.choice(data).values()) # a represents an user/account
    b = list(random.choice(data).values()) # b represents an user/account
...
# creating the variable tha will store the amount of followers from the variable above
    ...
    while play:
        ...
        if user_guess == 'a' and followers_a > followers_b: # checking the user's answers
            play = True
            total_score += 1
            b = list(random.choice(data).values())
            followers_b = b[1]
            ...
            # the B's information.
            b = list(random.choice(data).values()) # picking another user/account from the list
            followers_b = b[1]
        else:
            play = False
